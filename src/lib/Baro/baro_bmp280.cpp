...
void BMP280::initialize(uint8_t address)
{
    if (m_initialized)
        return;
    ...
    readRegister(address, BMP280_REG_CALIB_COEFFS_START, (uint8_t *)&m_calib, sizeof(m_calib));

    // configure IIR filter, 0.5ms standby time (freerunning)
    constexpr uint8_t BMP280_FILTER = FILTER_COEFF << 2;
    writeRegister(address, BMP280_REG_CONFIG, (uint8_t *)&BMP280_FILTER, sizeof(BMP280_FILTER));
...
void BMP280::startTemperature(uint8_t address)
{
    ...
    writeRegister(address, BMP280_REG_CTRL_MEAS, (uint8_t *)&BMP280_MODE, sizeof(BMP280_MODE));
}

uint32_t BMP280::getPressure(uint8_t address)
{
    return m_pressureLast;
}

int32_t BMP280::getTemperature(uint8_t address)
{
    // uint8_t status = 0xff;
    // readRegister(address, BMP280_REG_STATUS, &status, sizeof(status));
    // if (status & bit(3))
    // {
    //     DBGLN("not ready");
    // }
    ...
    readRegister(address, BMP280_REG_PRESSURE_MSB, buf, sizeof(buf));
...
bool BMP280::detect(uint8_t address)
{
    // Assumes Wire.begin() has already been called
    uint8_t chipid = 0;
    readRegister(address, BMP280_REG_CHIPID, &chipid, sizeof(chipid));
